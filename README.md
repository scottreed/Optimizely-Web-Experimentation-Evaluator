...
```bash
docker build -t optimizely-evaluator . && docker run --rm -p 8080:8080 optimizely-evaluator
```

The app will be available at `http://localhost:8080`.

To pass in your bearer token via environment variable:

```bash
docker build -t optimizely-evaluator . && docker run --rm -p 8080:8080 -e Optimizely__BearerToken=YOUR_TOKEN optimizely-evaluator
```